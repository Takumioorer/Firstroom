...
if( !empty($_POST['btn_confirm']) ) {
    ...
    if( !empty($clean['attachment_file']) ) {
        $body .= "Content-Type: application/ocset-stream; name\"
    ...
    //テキストメッセージをセット
    $body = "--__BOUNDARY__\n";
    $body .= "Contet-Type: text/plain; charset=\"ISO-2022-JP\"\n\n";
    $body .= $admin_reply_text . "\n";
    $body .= "--__BOUNDARY__\n";

    //ファイルを添付
    if( !empty($clean['attachment_file']) ) {
        $body .="Content-Type: application/ocset-stream; name=\"{$clean['attachment_file']}\"\n";
    ...
    //運営側へメールを送信
    mb_send_mail( 'webmaster@gray-code.com', $admin_reply_subject, $admin_reply_text, $body, $herder);
...
function validation($data) {
    ...
    } elseif( !preg_match( '/^[0-9a-z_.\/?-]+@([0-9a-z-]+\.)+[0-9a-z-]+$/',
    $data['email']) ) {
        $error[] = "「メールアドレス」を正しい形式で入力してください。";
    }
    ...
    } elseif( 140 < mb_strlen($data['contact']) ) {
...
<html lang="ja">
    ...
    <body>
        ...
        <form method="POST" action="">
            ...
            <?php if( !empty($clean['attachment_file']) ): ?>
                <input typ="hidden" name="attachment_file" value="<?php echo
                $clean['attachment_file']; ?>">
                <?php endif; ?>
        ...
        <form method="post" action="" enctype="multipart/form-data">
            ...
            <div class="element_wrap">
                ...
                value="male" <?php if( !empty($_POST['gender']) && $_POST['gender'] === "male" ){ echo
                'checked'; } ?>>男性</label>
                <label for="gender_female"><input id="gender_female" type="radio" name="gender"
                value="female" <?php if( !empty($_POST['gender']) && $_POST['gender'] === "female" ){
                echo 'checked'; } ?>>女性</label>
            </div>
            <div class="element_wrap"><label>年齢</label>
            <select name="age">
            ...
            <div class="element_wrap">
                <label>お問い合わせ内容</label>
                <textarea name="contact"><?php if( !empty($clean['contact']) ){ echo
            $clean['contact']; } ?></textarea>
            </div>
            <div class="element_wrap">
                <label>画像ファイルの添付</label>
                <input typ="file" name="attachment_file">
            </div>
            <div class="element_wrap">
                <label for="agreement"><input id="agreement" type="checkbox" name="agreement"
            value="1" <?php if( !empty($_POST['agreement']) && $_POST['agreement'] === "1" ){ echo
            'checked'; } ?>>プライバシーポリシーに同意する</label>
            </div>
